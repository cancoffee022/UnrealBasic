...
#pragma region ADD SIGHT PERCEPTION
	...
	// 시야 거리를 설정합니다
	AISightConfig->SightRadius = 400.f;

	// 시야 상실 거리를 설정합니다.
	AISightConfig->LoseSightRadius = 800.f;

	// 자극이 소멸되는 시간을 설정합니다
	AISightConfig->SetMaxAge(5.f);

	// 시야각을 설정합니다 (120' 감지)
	AISightConfig->PeripheralVisionAngleDegrees = 60.f;
...
#pragma endregion
	...
	// 시야에 어떠한 객체가 감지된 경우 발생 시킬 함수를 등록합니다
	GetPerceptionComponent()->OnTargetPerceptionUpdated.AddDynamic(this, &AKnightController::OnSightUpdated);
}
